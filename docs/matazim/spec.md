...
| REQ-M.8 | Return to intent | Hitting a member page while logged out lands on the מט״צים threshold and returns to the intended page afterwards, newly registered or freshly logged in. One link works for new and existing users alike; no branching is written anywhere. **Still TODO, and the code reads as though it is not, found 2026-09-12:** the login view already honours `request.POST.get("next")`, but no template ever sends it and `login_required`'s own `?next=` is dropped on the floor, so the line can never fire. Whoever implements this must also treat the destination as untrusted: it is only ever followed if it stays inside `/matazim/`, or return-to-intent becomes an open redirect out of the walls. | TODO |
...
| REQ-ID | Title | Expectation | Status |
|---|---|---|---|
| REQ-M.16 | Apply | Three questions only (grade, why, what have you built). The application is not what assesses them, the entrance test is, so every extra field is only a teenager who does not finish the form. Creates a `Student` row at `applied`. **Was marked DONE and is not, found 2026-09-12:** it also said "plus an `Application`", and there is no such table in either app. The form still requires the grade and the motivation, validates them, and then discards them, and the leader deciding whether to accept that person sees a name and an email. We make a fourteen-year-old write why they want to join and then throw it away, which is both a broken promise and a decision taken blind. The answers must be stored and shown to the leader who is being asked to say yes. | WIP |
...
| REQ-M.98 | A leader can be swapped | A student moves from one leader to another and the move is logged like every other transition (REQ-M.74). It is not rare: a teacher leaves, a child changes school, a pairing does not work. Nothing implements it today, and doing it by hand in Django's admin writes the field without the log, which is the one thing §4.7 says must not happen. | TODO |
