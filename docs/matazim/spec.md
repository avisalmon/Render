...
| REQ-M.143 | An institution can be handed to a successor | The tenancy root is a person (§4.4, §4.8), and the day she leaves her successor signs in to an empty programme. `manage.py matazim_handover old new --apply` moves every leader, event, invite and post she owns, atomically, and leaves history alone: a leader approved by her stays approved by her, and her requests stay hers because §4.11's log is her voice. The successor is made a program manager through `roles.grant_program_manager` (REQ-M.137); the old manager keeps her role, because revoking is its own decision (REQ-M.114). **Done 2026-09-14, as a bandage.** The real fix is the `Institution` row proposed in `data_model.md` §6, which is Avi's call. | DONE |
