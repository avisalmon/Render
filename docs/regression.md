...
## ustrip — family trip app (`tests/test_ustrip_access.py` 7)
...
screen-contract catalogue — so this is the one suite for now, covering the
one thing worth defending: the `family`-group access gate.

| Suite | Verifies |
|---|---|
| test_ustrip_access.py (7) | The family-group gate (spec §3): an anonymous visitor gets a plain access-denied page, never a 404; a logged-in non-member is denied the same way; a babook superuser who is not in `family` is still denied — admin status never implies access; an empty or missing `family` group locks out everyone, no exceptions; a `family` member gets in; the real seeded itinerary renders for a member (not fixture data); a bad itinerary id is a genuine 404 |
