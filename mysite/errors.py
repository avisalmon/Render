...
Django only allows one handler403/404/500 for the whole project, but two
isolated sub-products (matazim, ustrip) each need their own error pages so
neither leaks babook's chrome into their walls (REQ-M.2; ustrip spec §1).
This composes them by path prefix rather than teaching either app about the
other — matazim and ustrip stay unaware of each other's existence, exactly
as their own specs require. Anything outside both prefixes falls through to
Django's own defaults, which is what renders babook's own 404.html/500.html.
"""
